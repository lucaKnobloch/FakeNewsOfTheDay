    ...
    .mini-list
      table.mini
        ...
        tr(v-for="value,key in nodes")
          td {{ value.name }}
          td {{ value.content}}
          td <a :href ="value.url" target="_blank"> click for the article </a>
          td
            button.icon(@click='emit("unSelectNode",[value.id])' title="unselect") ❌
        //- disabled the function to see the selected links 
        //- tr(v-for="value,key in links")
        //-   td {{ value.sid }}
        //-   td {{ value.name }}
          td
            button.icon(@click='emit("unSelectLink",[value.id])' title="unselect") ❌
    ul.list
      li Nodes selected:
