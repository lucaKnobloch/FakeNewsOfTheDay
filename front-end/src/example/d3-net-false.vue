<template lang="pug">
  #example2(@keyup.esc='setTool("pointer")')
    //-> Network
    d3-network(
    ref='net2'
    :net-nodes="nodes"
    ...
    .over
      ...
      .options.menu(v-else)
        ul.inline
          normal <p> Entites classified as Fake </p>
        ul.inline
        <p> </p>
...
import D3NetMenu2 from "./Menu2.vue";
...
export default {
  ...
  watch: {
    myProp:function(val){
      this.selected = {};
      this.linksSelected = {};
      this.nodes = utils.makeFakeNodes(this.settings.maxNodes, val);
  ...
  methods: {
    ...
    reset() {
      this.selected = {};
      this.linksSelected = {};
      this.nodes = utils.makeFakeNodes(this.settings.maxNodes, this.date);
      this.lastNodeId = this.nodes.length + 1;
      this.links = utils.makeLinks2(this.nodes, this.settings.maxLinks);
      this.lastLinkId = this.links.length + 1;
    },
